 ...
 */
const generateRoute = (name = null, template = null, callback = null) => {
  return {
    name: name,
    /**
    ...
    action:function(params, queryParams) {
      if(callback) {
        callback(name, template, {params, queryParams})
      } else {
...
/**
 * Permet de générer une route "userconnecté".
 * Ces routes, sont des sous routes de /blog.
 * (La définition des routes admins est dans routing.js)
 * En gros, pour toutes les routes
 * @param template
 * @param callback
 * @returns {{action: action, triggersEnter: *[]}}
 */
const generateRouteUserConnected = (template, callback = null) => {
  return {
    /**
     * L'action qui va être effectué pour chaque accès a une route généré avec
     * cette fonction
     */
    action: function(params, queryParams) {
      if(callback) {
        callback(template, {params, queryParams})
      } else {
...
// Export des fonctions par un objet.
// Pour éviter de polluer les constantes globales avec nos petites fonctions.
RoutingUtils = {
  renderTemplate:renderTemplate,
  generateRoute:generateRoute,
  generateRouteUserConnected:generateRouteUserConnected
};