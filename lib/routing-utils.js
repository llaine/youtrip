...
/**
 * Permet de générer une route "administrateur".
 * Ces routes, sont des sous routes de /admin.
 ...
 */
const generateAdminRoute = (template, cb) => {
  return {
    /**
     * L'action qui va être effectué pour chaque accès à une route utilisateur
     */
    action: function() {
      if(cb) {
        cb(template)
...
RoutingUtils = {
  ...
  generateRouteAdmin:generateAdminRoute
};